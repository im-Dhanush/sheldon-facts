...
export default function Home() {
  ...
  // Get latest fact from backend
  useEffect(() => {
    async function fetchLatestFact() {
      try {
        const res = await fetch("/api/getLatestFact");
        const data = await res.json();
        if (data && data.fact) {
          setLatestFact(data);
        }
      } catch (err) {
        console.error("Error fetching latest fact:", err);
      }
    }
    fetchLatestFact();
  }, []);
            ...
            body: JSON.stringify({
              ...
              messages: [
                ...
                },
                {
                  role: "user",
                  content:
                    "Give me one fun fact and its explanation. Only one fact.",
                },
              ],
            }),
          }
  ...
  const saveFavorite = async () => {
    ...
    if (!token) {
      alert("Enable notifications first to use favorites!");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-black text-green-400 flex flex-col items-center p-6 font-mono">
      ...
      {/* Daily Fact Widget */}
      {latestFact && (
        <div className="w-full max-w-2xl bg-slate-800 border border-green-600 rounded-lg p-4 mb-6 shadow-md">
          <h2 className="text-green-500 font-semibold mb-2">
            📅 Yesterday’s Fact
          </h2>
          <p className="text-green-300 mb-2">🧐 {latestFact.fact}</p>
          {latestFact.explanation && (
            <p className="text-green-400 text-sm whitespace-pre-wrap">
              ✏️ {latestFact.explanation}
            </p>
          )}
        </div>
      )}
      ...
      >
        <div className="border-l-4 pl-4 border-pink-500 mb-4">
          ...
          <div className="text-green-300 mt-2 whitespace-pre-wrap break-words">
            {fact ||
              (loading
                ? "Fetching Sheldon-approved wisdom..."
                : "Click below for a fact.")}
          </div>
          {fullFact && (
            <button
              onClick={() => {
        ...
        {showExplanation && explanation && (
          <div className="mt-4 border border-green-600 rounded-md p-3 bg-slate-800">
            ...
            <div className="text-green-300 mt-2 whitespace-pre-wrap">
              {explanation}
            </div>
          </div>
        )}
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        ...
        {fact && (
          <button
            ...
            className="px-3 py-2 rounded bg-yellow-700 hover:bg-yellow-800 text-white"
          >
            ⭐ Save to Favorites
          </button>
        )}
